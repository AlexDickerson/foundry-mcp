...
export function registerActorTools(mcp: McpServer): void {
  mcp.registerTool('get_world_info', {
  ...
  mcp.registerTool('create_actor', {
    ...
    inputSchema: {
      ...
      system: z.record(z.string(), z.unknown()).optional().describe('System-specific data (e.g. PF2e ability scores)'),
    },
  }, async ({ name, type, folder, img, system }): Promise<CallToolResult> =>
    foundryTool('create-actor', { name, type, folder, img, system }),
  );

  mcp.registerTool('create_actor_from_compendium', {
  ...
  mcp.registerTool('delete_actor', {
    ...
    foundryTool('delete-actor', { actorId }),
  );
}
