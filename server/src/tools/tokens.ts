...
export function registerTokenTools(mcp: McpServer): void {
  ...
  mcp.registerTool('move_token_path', {
  ...
  mcp.registerTool('stop_patrol', {
    title: 'Stop Token Patrol',
    description: 'Stop a token\'s active patrol.',
    inputSchema: {
      tokenId: z.string().describe('Token ID to stop patrolling'),
    },
  }, async ({ tokenId }): Promise<CallToolResult> =>
    foundryTool('stop-patrol', { tokenId }),
  );

  mcp.registerTool('get_patrols', {
    ...
    foundryTool('get-patrols', { sceneId }),
  );
}
