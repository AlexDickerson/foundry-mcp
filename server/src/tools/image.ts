  ...
  mcp.registerTool('edit_image', {
    ...
    try {
      ...
      if (image) {
        ...
        const result = await sendCommand('get-scene-background', { sceneId, maxDimension: 2048 }) as Record<string, unknown>;
        if (!result?.image) {
          return { content: [{ type: 'text', text: 'Error: Could not fetch scene background image.' }], isError: true };
        }
      ...
      return {
        content: [
          { type: 'image', data: editedData, mimeType: 'image/png' },
        ],
      };
    } catch (err) {
