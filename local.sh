...
Commands:
  ...
  rebuild    Rebuild module + server dist locally and restart the container
  logs       Tail container logs
  shell      Open a bash shell in the running container
  status     Show container status + MCP health
  ps         Compose status

First-time:
  1. Copy .env.example to .env and fill in FOUNDRY_USERNAME/PASSWORD/OPENAI_API_KEY
  2. ./local.sh up
...
build_local() {
  ...
  cp -r module/dist/module.json module/dist/styles module/dist/templates module/dist/ 2>/dev/null || true
...
cmd_status() {
  ...
  curl -fsS "http://localhost:${port}/health" 2>/dev/null || echo "  not reachable (container may still be starting)"
}
